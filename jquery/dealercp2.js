...
// var brandlist = new Array("Porsche", "Porsche", "Porsche", "Porsche");
let number_car = [1, 1, 1, 1];
let stati = [0, 0, 0];
let sum = 0,
  temp_sum = 0;
let i = 0;
...
function move_ke_car_yang_lain(cus_dalam, car) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      console.log(car);
      setTimeout(() => {
        bergerak_ke_kereta_spesifik(cus_dalam, car);
      }, 1000);
      setTimeout(() => {
        buat_buat_pilihan(cus_dalam, car);
      }, 1000);
      resolve(0);
    }, 1000);
  });
...
async function move_ke_car(get_car, cus_dalam) {
  ...
  if (get_car == "Porsche") {
    ...
    await bergerak_ke_kereta_spesifik(cus_dalam, get_car);
    (await number_car[0]) == 0
      ? move_ke_car_yang_lain(cus_dalam, get_car)
      : buat_buat_pilihan(cus_dalam, get_car);

    // if (number_car[0] == 0) {
    //   await move_ke_car_yang_lain(cus_dalam, get_car);
    // }else{
    //   await buat_buat_pilihan(cus_dalam, get_car);
    // }
